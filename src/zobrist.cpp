...
void init_zobrist() {
    ...
    for (int c = WHITE; c <= BLACK; c++) {
        for (int p = PAWN; p <= KING; p++) {
            for (int sq = 0; sq < 64; sq++) { zobrist_piece[c][p][sq] = rng(); }
        }
    }
    ...
    for (int i = 0; i < 64; i++) { zobrist_en_passant[i] = rng(); }
}
...
uint64_t generate_hash(const Board &board) {
    ...
    if (board.side_to_move == BLACK) { hash ^= zobrist_side; }

    hash ^= zobrist_castling[board.castling_rights];

    if (board.en_passant != SQ_NONE) { hash ^= zobrist_en_passant[board.en_passant]; }
