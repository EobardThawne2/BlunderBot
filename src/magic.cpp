...
void init_sliders(bool bishop) {
    for (int sq = 0; sq < 64; sq++) {
        ...
        for (int i = 0; i < occupancy_indicies; i++) {
            occupancies[i] = set_occupancy(i, relevant_bits, mask);
            attacks_on_the_fly[i] = bishop ? bishop_attacks_on_the_fly(sq, occupancies[i]) : rook_attacks_on_the_fly(sq, occupancies[i]);
        }

        uint64_t magic = find_magic_number(sq, relevant_bits, bishop);
        if (bishop) bishop_magic_numbers[sq] = magic;
        else rook_magic_numbers[sq] = magic;

        int shift = 64 - relevant_bits;
        for (int i = 0; i < occupancy_indicies; i++) {
            int magic_index = (occupancies[i] * magic) >> shift;
            if (bishop) bishop_attacks[sq][magic_index] = attacks_on_the_fly[i];
            else rook_attacks[sq][magic_index] = attacks_on_the_fly[i];
        }
    }
}
